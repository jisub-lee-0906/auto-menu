...
This project uses Korean institutional meal planning conventions.
Each item should be stored in the single category that best matches how a school or cafeteria nutritionist would present it on a menu board.
...
- Examples: `현미밥`, `비빔밥`, `덮밥`, `볶음밥`, `죽`, `주먹밥`, `김밥`.

### `soup`

- Soups, broths, stews, tang, guk, jjigae, noodles served as soup dishes, and soup-style siders.
- Examples: `미역국`, `된장찌개`, `갈비탕`, `칼국수`, `우동국`, `수제비국`.

### `main`

- Main protein dish or dominant entree served as the primary side.
- Includes meat, fish, poultry, tofu-based entree, substantial fried or grilled item, and entree-style convenience foods.
- Examples: `불고기`, `치킨`, `생선까스`, `족발`, `함박`, `탕수육`, `카츠`, `떡갈비`.
...
- Fruit, yogurt, milk, beverage, bread/snack dessert, jelly, pudding, ice cream, and sweet item served after the meal.

## Decision Rules

1. Prefer how the item is displayed on a real Korean school lunch menu, not just the recipe type.
2. If an item is clearly protein-dominant and could anchor the meal, it belongs in `main`.
...
5. Generic ingredient names or unclear truncated names should not remain in the dataset.
